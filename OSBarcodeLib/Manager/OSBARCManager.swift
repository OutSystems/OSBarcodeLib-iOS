...
extension OSBARCManager: OSBARCManagerProtocol {
    func scanBarcode() async throws -> String {
        ...
        return try await withCheckedThrowingContinuation { self.startScanning(continuation: $0) }
    }
    
    /// Triggers the scanner view.
    /// - Parameter continuation: Object responsible for returning the method's result to its caller.`
    private func startScanning(continuation: CheckedContinuation<String, any Error>) {
        DispatchQueue.main.async {
            self.scannerBehaviour.startScanning { scannedCode in
